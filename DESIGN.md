...
界面像一枚悬浮在暗房中的光学镜片：冷静、克制、透光，但不会牺牲专业工具所需的清晰度和密度。一级页面用影像、留白和按需出现的高层级账户玻璃建立品牌气氛；工作台则把同一种材质压缩为更紧凑的控制表面。
...
一级页面在桌面端以单个动态视口承载首页、能力、隐私和联系方式四个独立场景；前进时当前场景向左退出、下一场景从右进入，后退时方向相反。账户表单默认不占据画面，只由右上角“登录工作台”打开居中液态玻璃弹窗。移动导航折叠为全屏菜单。工作台在宽屏维持参考区、画布区、检查器三区结构，在平板隐藏非关键列，在手机按操作顺序纵向排列。
...
- **环境悬浮:** 低透明度的大半径黑色阴影，应用于一级账户弹窗和云端历史。
...
主控件使用 15px 圆角，工作台面板使用约 20px 圆角，一级账户玻璃弹窗使用 32px 圆角。胶囊只用于状态、标签和分段控制。品牌镜片标记允许轻微不规则轮廓，其余组件保持稳定几何形状。
...
一级账户弹窗是系统的签名组件：不对称内部光、32px 圆角、增强饱和模糊、顶部高光和独立滚动共同确保视频复杂时表单仍清晰。它只在用户主动点击“登录工作台”后出现，页面上不重复放置第二个确认入口。

### Workspace Reveal

登录后按顶部工具栏、参考图库、主画布、强度控制、待调色队列、右侧检查器的顺序短促显现。只使用 `opacity` 和 `transform`，间隔约 70ms；滑杆、曲线、直方图及其内部控件不做级联动画，确保实时调色性能。

### Import Progress

全局导入状态必须展示当前文件、真实字节读取量、文件序号、当前处理阶段和总体百分比。阶段文案区分读取、预览解码、语义识别与颜色档案构建；移动端使用同一紧凑玻璃卡，不以无限旋转动画代替可验证的进度。
